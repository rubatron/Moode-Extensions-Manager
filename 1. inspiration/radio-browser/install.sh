...
# Security/runtime principal settings
...
detect_default_ssh_user() {
    ...
    if user_exists moode; then
        echo "moode"
        return 0
    fi

    if user_exists pi; then
        echo "pi"
        return 0
    fi

    local any_home
    any_home=$(awk -F: '$6 ~ /^\/home\// {print $1; exit}' /etc/passwd)
    echo "$any_home"
}
...
setup_security_principal() {
    ...
    if ! group_exists "$SECURITY_GROUP"; then
        groupadd --system "$SECURITY_GROUP"
        info "Created group: $SECURITY_GROUP"
    fi

    if ! user_exists "$SECURITY_USER"; then
        useradd --system --no-create-home --shell /usr/sbin/nologin --gid "$SECURITY_GROUP" "$SECURITY_USER"
        info "Created user: $SECURITY_USER"
    fi

    local ssh_user
    ssh_user="$(detect_default_ssh_user)"

    if [[ -n "$ssh_user" ]] && user_exists "$ssh_user"; then
        info "Syncing supplementary groups from SSH user: $ssh_user"
        local grp
        for grp in $(id -nG "$ssh_user"); do
            add_user_to_group_safe "$SECURITY_USER" "$grp" || true
        done
    else
        warning "Could not detect default SSH user; skipping SSH-group sync"
    fi

    # Ensure extension account can cooperate with web runtime accounts.
    add_user_to_group_safe "$SECURITY_USER" "$WEB_GROUP" || true
    add_user_to_group_safe "$WEB_USER" "$SECURITY_GROUP" || true

    # Optional nginx naming on some distros.
    if group_exists nginx; then
        add_user_to_group_safe "$SECURITY_USER" nginx || true
    fi

    # Optional MySQL access where present.
    if group_exists mysql; then
        add_user_to_group_safe "$SECURITY_USER" mysql || true
    fi

    success "Security principal configured"
    return 0
}

grant_database_access() {
    log "Applying database access policy..."

    if [[ -f "$SQLITE_DB" ]]; then
        if command -v setfacl > /dev/null 2>&1; then
            setfacl -m "u:${SECURITY_USER}:rw" "$SQLITE_DB" || true
            setfacl -m "u:${SECURITY_USER}:rwx" "$(dirname "$SQLITE_DB")" || true
            info "Granted SQLite access via ACL: $SQLITE_DB"
        else
            chgrp "$SECURITY_GROUP" "$SQLITE_DB" || true
            chmod g+rw "$SQLITE_DB" || true
            info "Granted SQLite access via group fallback: $SQLITE_DB"
        fi
    else
    ...
    if [[ -S "$MYSQL_SOCKET" ]]; then
        if group_exists mysql; then
            add_user_to_group_safe "$SECURITY_USER" mysql || true
            info "MySQL socket detected and mysql group mapped"
        fi
    fi

    success "Database access policy applied"
    return 0
}

install_watchdog_service() {
    log "Installing extmgr watchdog service..."
...
log() {
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] [extmgr-watchdog] $1" >> "$LOG_FILE"
}

while true; do
    if [[ ! -d "$EXT_BASE" ]]; then
        log "extension path missing: $EXT_BASE"
    fi

    if [[ -d "$EXT_BASE/cache" ]] && [[ ! -w "$EXT_BASE/cache" ]]; then
        log "cache not writable: $EXT_BASE/cache"
    fi
...
EOF
    ...
    systemctl enable --now "$(basename "$WATCHDOG_SERVICE")" || true
...
set_permissions() {
    ...
    # Set ownership to dedicated extension principal
    chown -R "${SECURITY_USER}:${SECURITY_GROUP}" "${EXT_BASE}"
    chown -h "${SECURITY_USER}:${SECURITY_GROUP}" "${WEB_ROOT}/radio-browser.php" 2>/dev/null || true
    ...
    find "${EXT_BASE}/scripts" -type f -name "*.sh" -exec chmod 755 {} \; 2>/dev/null || true

    # Writable runtime dirs with setgid so new files inherit security group
    chmod 2775 "${CACHE_DIR}" 2>/dev/null || true
    chmod 2775 "${IMAGE_CACHE_DIR}" 2>/dev/null || true
    chmod 2775 "${DATA_DIR}" 2>/dev/null || true

    find "${CACHE_DIR}" -type f -exec chmod 664 {} \; 2>/dev/null || true
    find "${DATA_DIR}" -type f -exec chmod 664 {} \; 2>/dev/null || true

    # Optional ACL for web runtime in distros where group propagation is delayed.
    if command -v setfacl > /dev/null 2>&1; then
        setfacl -R -m "u:${WEB_USER}:rwX" "${EXT_BASE}" 2>/dev/null || true
        setfacl -R -d -m "u:${WEB_USER}:rwX" "${CACHE_DIR}" "${DATA_DIR}" 2>/dev/null || true
    fi
...
auto_install() {
    ...
    # Step 4: Setup security principal and group model
    echo -e "${BOLD}Step 4/10: Configuring security principal...${NC}"
    setup_security_principal || { error "Failed to configure security principal"; ((errors++)); }
    echo

    # Step 5: Create backup
    echo -e "${BOLD}Step 5/10: Creating backup...${NC}"
    create_backup || warning "Backup creation issue"
    echo

    # Step 6: Create folders
    echo -e "${BOLD}Step 6/10: Creating folders...${NC}"
    create_folders || { error "Failed to create folders"; ((errors++)); }
    echo

    # Step 7: Copy files
    echo -e "${BOLD}Step 7/10: Copying files...${NC}"
    copy_files || { error "Failed to copy files"; ((errors++)); }
    echo

    # Step 8: Set permissions
    echo -e "${BOLD}Step 8/10: Setting permissions...${NC}"
    set_permissions || { error "Failed to set permissions"; ((errors++)); }
    echo

    # Step 9: Grant DB access profile
    echo -e "${BOLD}Step 9/10: Applying database access policy...${NC}"
    grant_database_access || warning "Database access policy issue"
    echo

    # Step 10: Install watchdog service
    echo -e "${BOLD}Step 10/10: Installing watchdog service...${NC}"
    install_watchdog_service || warning "Watchdog service issue"
    echo
...
show_menu() {
    ...
    echo "  8. Configure security principal"
    echo "  9. Apply database access policy"
    echo
    echo -e "${BOLD}Maintenance:${NC}"
    echo "  0. Restart services (PHP-FPM, nginx)"
    echo "  u. Uninstall extension"
    echo
...
main() {
    ...
    case "${1:-}" in
        ...
        "")
            ...
            ;;
        *)
            echo "Usage: $0 [--auto|-a] [--uninstall|-u] [--help|-h] [--status|-s] [--security]"
            echo
            echo "Options:"
            ...
            echo "  --security       Configure security user/group + DB + watchdog"
