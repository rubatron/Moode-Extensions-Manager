...
MODULE_MENU_SCRIPT="${SCRIPT_DIR}/scripts/install-modules.sh"
...
launch_modules_menu() {
    echo
    if [[ ! -f "$MODULE_MENU_SCRIPT" ]]; then
        error "Module installer not found: $MODULE_MENU_SCRIPT"
        return 1
    fi

    log "Launching module installer menu..."
    bash "$MODULE_MENU_SCRIPT"
}
...
show_menu() {
    ...
    echo "  m. Module installer menu (Module 1/2/3)"
    ...
    while true; do
        ...
        case $choice in
            ...
            h|H)
                ...
                ;;
            m|M)
                launch_modules_menu || warning "Module installer exited with an error"
                press_any_key
                ;;
            s|S)
                check_installation
...
main() {
    ...
    case "${1:-}" in
        ...
        "")
            ...
            ;;
        *)
            echo "Usage: $0 [--auto|-a] [--uninstall|-u] [--modules|-m] [--help|-h] [--status|-s]"
            echo
            echo "Options:"
            echo "  --auto, -a       Run automatic installation"
            echo "  --uninstall, -u  Uninstall the extension"
            echo "  --modules, -m    Open module installer menu (Module 1/2/3)"
