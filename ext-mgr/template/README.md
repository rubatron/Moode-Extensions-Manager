...
## boot_config (Hardware Configuration)

Extensions that require hardware configuration (DAC overlays, GPIO settings) can use boot_config:
...
Lines are added to `/boot/firmware/config.txt` via ext-mgr's boot_config helper.
Configuration is automatically removed when the extension is uninstalled.
